        ...
        public static void onMobDrops(LivingDropsEvent event) {
            ...
            if (event.getEntity() instanceof Mob killedMob) {
                if (Config.COMMON.dropChance_bag_of_seeds.get() > 0
                        && event.getEntity().getRandom().nextFloat() < Config.COMMON.dropChance_bag_of_seeds.get() + (event.getLootingLevel()) * 0.01f) {
            ...
            if (event.getEntity() instanceof Mob killedMob) {
                if (Config.COMMON.dropChance_bag_of_saplings.get() > 0
                        && event.getEntity().getRandom().nextFloat() < Config.COMMON.dropChance_bag_of_saplings.get() + (event.getLootingLevel()) * 0.01f) {
                    event.getDrops().add(new net.minecraft.world.entity.item.ItemEntity(
                            event.getEntity().level(),
                            killedMob.getX(),
                            killedMob.getY(),
